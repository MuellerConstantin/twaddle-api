...
Some endpoints allow filtering of the resource. Filtering is done using the
https://github.com/jirutka/rsql-parser#grammar-and-semantic[RSQL] query language. For this purpose,
an RSQL query via HTTP query parameter `filter` is sent with the actual request.
...
|filter
|Contains the RSQL query used for filtering.
|true
|===

CAUTION: Only a subset of the possible RSQL operations are supported. This relates in particular to the comparison operators.
...
NOTE: Accounts may be blocked for repeated violations, causing authentication to fail.
...
|Used when a client tries to send a message before it joined a room. Sending messages outside of a room is not possible.
